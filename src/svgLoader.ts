...
function createTrainMove(data: SvgParsedData): TrainMove {
  ...
  if (false) {
    for (const track of data.tracks) {
      const [track1, track2, switches_] = createNewTrack(track._begin, track._end, [], [], null);
  ...
  // 横長の連続する線路をマージする
  {
    ...
    while (data.tracks.length > 0) {
      ...
      if (track._begin.y !== track._end.y) {
        ...
        const tracksOnSameLine = data.tracks.filter(
          (t) => t !== track && t._begin.y === track._begin.y && t._end.y === track._begin.y
        );
  ...
  for (const rect of data.platforms) {
    // TODO: 絞りこむ
    const adjacentTracks = data.tracks
  ...
  for (const track of data.tracks.filter((t) => t._begin.y !== t._end.y)) {
    if (
      data.tracks.filter((t) => t !== track && (deepEqual(t._begin, track._begin) || deepEqual(t._end, track._begin)))
        .length === 0
    ) {
      const horizontalTracks = data.tracks.filter(
        (t) =>
          t._begin.y === track._begin.y &&
          t._end.y === track._begin.y &&
          t._begin.x < track._begin.x &&
          t._end.x > track._begin.x
      );
    ...
    if (
      data.tracks.filter((t) => t !== track && (deepEqual(t._begin, track._end) || deepEqual(t._end, track._end)))
        .length === 0
    ) {
      const horizontalTracks = data.tracks.filter(
        (t) =>
          t._begin.y === track._end.y &&
          t._end.y === track._end.y &&
          t._begin.x < track._end.x &&
          t._end.x > track._end.x
      );
      if (horizontalTracks.length > 0) {
        const horizontalTrack = horizontalTracks[0];
        data.tracks.push({
  ...
  while (platformCandidateSet.size > 0) {
    ...
    const adjacentRects = data.platforms.filter(
      (p) => p.position.x + p.width > rect.position.x && rect.position.x + rect.width > p.position.x
    );
    const adjacentTracks = data.tracks
      .filter((p) => p._end.x > rect.position.x && rect.position.x + rect.width > p._begin.x)
      .filter((p) => p._begin.y === p._end.y);
    const adjacentTexts = data.texts.filter(
      (p) => p.position.x > rect.position.x && rect.position.x + rect.width > p.position.x
    );
    ...
    stationTracks.forEach((stationTrack, trackIndex) => {
      const tracks_ = trainMove.tracks.filter(
        (track) => deepEqual(track._begin, stationTrack._begin) && deepEqual(track._end, stationTrack._end)
      );
