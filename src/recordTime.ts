...
export function loadTime() {
  ...
  fetch('./saved_time.json')
    .then((data) => data.json())
    .then((savedData: TimedPositionData) => {
      ...
      function moveSeekBar(mouseX: number) {
        trainMove.globalTime = Math.round(
          (savedData.maxGlobalTime - savedData.minGlobalTime) * (mouseX / seekBarWidth) + savedData.minGlobalTime
        );
