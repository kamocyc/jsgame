...
import { Operation, Station, Train } from "../../model";
...
export function DiagramOperationComponent(props: DiagramOperationProps) {
  ...
  return (<ListSettingCommonComponent<Operation>
    ...
    getSettingComponent={(operation) => {
      return (<DiagramOperationSubComponent
        {...props}
        operation={operation}
        />)
    }}
      ...
      <tbody>
        {operation.trains.map((train, i) => {
          ...
          const [diaTime1, direction, diaTime2] =
            train.direction === 'Inbound' ? [train.diaTimes[0], '→', train.diaTimes[train.diaTimes.length - 1]] : [train.diaTimes[train.diaTimes.length - 1], '←', train.diaTimes[0]];
          const [time1, time2] =
            train.direction === 'Inbound' ? [diaTime1.departureTime, diaTime2.arrivalTime] : [diaTime1.arrivalTime, diaTime2.departureTime]
          return (<tr key={i}>
